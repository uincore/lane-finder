...
class LaneValidator:
    ...
    def validate(self, lane, allow_line_projection=False):
        ...
        if lane_width_is_valid:
            ...
            if allow_line_projection and left_line_is_valid != right_line_is_valid:
                return ValidationResult(True, left_line_is_valid, right_line_is_valid, car_is_on_lane)

        return ValidationResult(False, left_line.is_valid, right_line.is_valid)


class ValidationResult:

    def __init__(self, lane_is_valid, left_line_is_valid, right_line_is_valid, car_is_on_lane=False, width_deviation=0):
        self.lane_is_valid = lane_is_valid
        self.left_line_is_valid = left_line_is_valid
        self.right_line_is_valid = right_line_is_valid
